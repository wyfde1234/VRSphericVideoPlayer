...
struct TextureBuffer
{
	...
	TextureBuffer(GLsizei width, GLsizei height, int mipLevels, unsigned char *data, int sampleCount) :
		texId(0)
	{
...
struct ShaderFill
{
	...
	ShaderFill(GLuint vertexShader, GLuint pixelShader, TextureBuffer* _texture)
	{
		...
		if (!r)
		{
			...
			dprintf(("Linking shaders failed: %s\n", msg));
		}
	}
...
struct VertexDataScene
{
	...
	Vector2 texCoord;
	VertexDataScene() {};
	VertexDataScene(Vector3 pos, DWORD c, Vector2 tex) : position(pos), color(c), texCoord(tex) {};
};

struct Model
{
	GLuint          sceneVAO;
	GLuint          glSceneVertBuffer;
	GLuint          glSceneIndexBuffer;
	ShaderFill      *Fill;
	std::vector<GLuint> indexarray;
	std::vector<VertexDataScene> vertdataarray;

	Model(ShaderFill *fill) :
		sceneVAO(0),
		glSceneVertBuffer(0),
		glSceneIndexBuffer(0),
		Fill(fill),
		vertdataarray(),
		indexarray()
	{
	...
	void ChangeVideoFrame(int width, int height, unsigned char* frame)
	{
		delete Fill->texture;
		Fill->texture = new TextureBuffer(width, height, 4, frame, 1);
	}

	void AddVertexVal(const VertexDataScene ft) { vertdataarray.push_back(ft); }

	void AddIndex(GLuint a) { indexarray.push_back(a); }
	...
	void AllocateBuffers()
	{
		...
		glGenBuffers(1, &glSceneVertBuffer);
		glBindBuffer(GL_ARRAY_BUFFER, glSceneVertBuffer);
		glBufferData(GL_ARRAY_BUFFER, sizeof(VertexDataScene) * vertdataarray.size(), &vertdataarray[0], GL_STATIC_DRAW);

		glGenBuffers(1, &glSceneIndexBuffer);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, glSceneIndexBuffer);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLuint) * indexarray.size(), &indexarray[0], GL_STATIC_DRAW);

		glEnableVertexAttribArray(0);
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(VertexDataScene), (void *)offsetof(VertexDataScene, position));

		glEnableVertexAttribArray(1);
		glVertexAttribPointer(1, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(VertexDataScene), (void *)offsetof(VertexDataScene, color));

		glEnableVertexAttribArray(2);
		glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(VertexDataScene), (void *)offsetof(VertexDataScene, texCoord));
	...
	void FreeBuffers()
	{
		...
		glDeleteBuffers(1, &glSceneVertBuffer);
		glDeleteBuffers(1, &glSceneIndexBuffer);
		if (sceneVAO != 0)
		{
	...
	void Render(const float* mat)
	{
		...
		glUniformMatrix4fv(glGetUniformLocation(Fill->program, "matrix"), 1, GL_FALSE, mat);
		glBindVertexArray(sceneVAO);
...
struct CaptureScene {
	...
	static const GLushort Slices = 36;
	GLuint CreateShader(GLenum type, const GLchar* src)
	{
		...
		if (!r)
		{
			...
			if (msg[0]) {
				dprintf(("Compiling shader failed: %s\n", msg));
			}
			return 0;
		}
	...
	CaptureScene(int width, int height, unsigned char* frame) :
		videoWidth(width), videoHeight(height)
	{
		static const GLchar* VertexShaderSrc =
			...
			"layout(location = 2) in vec2 v2UVcoordsIn;\n"
			"layout(location = 3) in vec3 v3NormalIn;\n"
			"out vec2 v2UVcoords;\n"
			"out vec4 oColor;\n"
			"void main()\n"
			"{\n"
		...
		static const char* FragmentShaderSrc =
			...
			"void main()\n"
			"{\n"
			"   outputColor = oColor * texture(mytexture, v2UVcoords);\n"
			"}\n";
		...
		{
			float phi = M_PI / 2 - stack *  M_PI / Stacks;
			...
			for (int slice = 0; slice <= Slices; slice++)
			{
				...
				float x = scale * sin(theta);
				float z = scale * cos(theta);
				m->AddVertexVal(VertexDataScene(Vector3(x, y, z), 0xffffffff, Vector2(1.0f - slice / (double)Slices, 2 * stack / (double)Slices)));
				if (stack != Stacks && slice != Slices)
				{
					if (stack != 0)
					{
						m->AddIndex(top + slice);
						m->AddIndex(bot + slice);
						m->AddIndex(top + slice + 1);
					}
					if (stack != Stacks - 1)
					{
						m->AddIndex(top + slice + 1);
						m->AddIndex(bot + slice);
						m->AddIndex(bot + slice + 1);
					}
				}
...
#else
		//Large screen
		VertexDataScene vvvs[4] = { VertexDataScene(Vector3(-(float)videoWidth / 2 * .02, (float)videoHeight / 2 * .02, 0), 0xffffffff, Vector2(1, 0)),
			VertexDataScene(Vector3((float)videoWidth / 2 * .02, (float)videoHeight / 2 * .02, 0), 0xffffffff, Vector2(0, 0)),
			VertexDataScene(Vector3(-(float)videoWidth / 2 * .02, -(float)videoHeight / 2 * .02, 0), 0xffffffff, Vector2(1, 1)),
			VertexDataScene(Vector3((float)videoWidth / 2 * .02, -(float)videoHeight / 2 * .02, 0), 0xffffffff,Vector2(0, 1)) };
		GLuint CubeIndices[] =
		{
			0, 2, 1, 1, 2, 3
		};
	...
	void Render(unsigned char *frame, const float *mat) {
		// Construct geometry
...
{
	libvlc_video_get_size(pMediaPlayer_, 0, &VideoWidth, &VideoHeight);
...
bool CMainApplication::HandleInput()
{
	...
	while (SDL_PollEvent(&sdlEvent) != 0)
	{
		...
		else if (sdlEvent.type == SDL_KEYDOWN)
		{
			...
			{
				int64_t Position = libvlc_media_player_get_time(pMediaPlayer_);
				Position += 10000;
				libvlc_media_player_set_time(pMediaPlayer_, Position);
			}
			...
			{
				bPaused = !bPaused;
				libvlc_media_player_set_pause(pMediaPlayer_, bPaused);
			}
		}
	}
...
bool CMainApplication::getFileToPlay()
{
...
void CMainApplication::SetupScene()
{
	...
	libvlc_video_set_callbacks(pMediaPlayer_, lock, unlock, display, NULL);

	pVideo = new unsigned char[m_nRenderWidth * m_nRenderHeight * 4];
	memset(pVideo, 0, sizeof(pVideo));

	roomScene = new CaptureScene(m_nRenderWidth, m_nRenderHeight, pVideo);
}
...
void CMainApplication::RenderControllerAxes()
{
	...
	if (!m_pHMD || !m_pHMD->IsInputAvailable())
		return;
...
bool CMainApplication::SetupStereoRenderTargets()
{
	...
	VideoBuffer = new DWORD[m_nRenderWidth * m_nRenderHeight];
...
void CMainApplication::RenderScene(vr::Hmd_Eye nEye)
{
	...
	if (m_bShowCubes)
	{
		if (bUpdateFrame)
		{
			memcpy(pVideo, VideoBuffer, m_nRenderWidth * m_nRenderHeight * 4);
			//std::vector<std::uint8_t> ImageBuffer;
			//lodepng::encode(ImageBuffer, pVideo, m_nRenderWidth, m_nRenderHeight);
			//lodepng::save_file(ImageBuffer, "d:/1.png");
			bUpdateFrame = false;
		}

		roomScene->Render(pVideo, GetCurrentViewProjectionMatrix(nEye).get());
	}
...
void CMainApplication::UpdateHMDMatrixPose()
{
	...
	if (m_rTrackedDevicePose[vr::k_unTrackedDeviceIndex_Hmd].bPoseIsValid)
	{
		...
		m_mat4HMDPose *= LookAt; // set up viewport
...
int main(int argc, char *argv[])
{
	...
	if (!pVLCInstance_)
	{
		MessageBox(NULL, "Failed to Create VLC", "Error", MB_OK);
		return FALSE;
	}
